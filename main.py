...
@click.command()
@click.option('--bucketname', default=None, help='Specify S3 bucket name. If omitted, retrieves available buckets.')
...
def main(bucketname, region, size_unit, storage_class, version):
    """Entrypoint for the S3 monitoring tool."""
    s3_client = S3Collector("").s3_client

    if version:
        print(f"S3 Monitor - Version {VERSION}")
        return
